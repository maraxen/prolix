...
import numpy as np
import logging
import msgpack # For array_record if needed, though usually we save analysis data only or occasional frames
...
def run_replica_exchange(
    ...
    if spec.use_pbc:
        if spec.box is None:
             raise ValueError("Box required for PBC")
    ...
    if r_init.ndim == 2:
        r_replicas = jnp.tile(r_init[None, ...], (n_replicas, 1, 1))
    else:
        r_replicas = r_init
    ...
    v_init_fn = jax.vmap(init_fn, in_axes=(0, 0, 0)) # keys, Rs, kTs
    ...
    # 3. Main Loop
    steps_per_exchange = int(round(spec.exchange_interval_ps / (spec.step_size_fs * 1e-3)))
    ...
    for i in range(total_intervals):
        (state, key), snapshot = run_interval(curr_carrier)
