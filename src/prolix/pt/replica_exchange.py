...
class ReplicaExchangeState(eqx.Module):
  ...
  def numpy(self) -> dict[str, Any]:
    ...
    return jax.tree_util.tree_map(
      lambda x: np.asarray(jax.device_put(x, jax.devices("cpu")[0])), dataclasses.asdict(self)
    )
...
def run_replica_exchange(
  ...
  if constrained_bonds is not None and len(constrained_bonds) > 0:
    constraints = (jnp.asarray(constrained_bonds), jnp.asarray(lengths))
  ...
  def init_wrapper(k, r, t, m):
    return init_fn(k, r, kT=t, mass=m)  # type: ignore[unknown-argument]
  ...
  def step_wrapper(s, t):
    return apply_fn(s, kT=t)  # type: ignore[unknown-argument]
