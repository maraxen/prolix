...
def _fit_efa_params(
    ...
    # Target function: erf(αr)/r over the fit range
    r = np.linspace(r_min, r_max, n_pts)
    target = scipy_erf(alpha * r) / r  # shape (n_pts,)

    # Frequency grid: K frequencies from 0 to ω_max
    # ω_max = 2π / r_min ensures oscillation at the shortest relevant distance
    omega_max = 2.0 * np.pi / r_min
    ...
    # Non-negative least squares to ensure w_k >= 0
...
def efa_lebedev_params(
    ...
    Results are cached — computed once per (alpha, n_freqs, n_lebedev_pts) triple.
    ...
    omegas, weights = _fit_efa_params(alpha, n_freqs)
...
def efa_erf_features(
    ...
    # Feature amplitude per (node, freq): sqrt(w_k * λ_j * 4π)
    amplitude = jnp.sqrt(weights[None, :] * lam[:, None] * 4.0 * jnp.pi)  # (G, K)
