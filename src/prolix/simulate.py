...
def run_simulation(
  system: "AtomicSystem | SystemParams",
  ...
  Args:
      ...
      r_init: Initial positions. If None, uses system.coordinates (for AtomicSystem).
  ...
  if hasattr(system, 'coordinates') and hasattr(system, 'charges'):
      ...
      system_params_dict: SystemParams = {
          "charges": jnp.array(atomic_sys.charges) if atomic_sys.charges is not None else jnp.zeros(n_atoms),
          "sigmas": jnp.array(atomic_sys.sigmas) if atomic_sys.sigmas is not None else jnp.ones(n_atoms) * 3.0,
          "epsilons": jnp.array(atomic_sys.epsilons) if atomic_sys.epsilons is not None else jnp.ones(n_atoms) * 0.1,
          "bonds": jnp.array(atomic_sys.bonds) if atomic_sys.bonds is not None else jnp.zeros((0, 2), dtype=jnp.int32),
          "bond_params": jnp.array(atomic_sys.bond_params) if atomic_sys.bond_params is not None else jnp.zeros((0, 2)),
          "angles": jnp.array(atomic_sys.angles) if atomic_sys.angles is not None else jnp.zeros((0, 3), dtype=jnp.int32),
          "angle_params": jnp.array(atomic_sys.angle_params) if atomic_sys.angle_params is not None else jnp.zeros((0, 2)),
          "dihedrals": jnp.array(atomic_sys.proper_dihedrals) if atomic_sys.proper_dihedrals is not None else jnp.zeros((0, 4), dtype=jnp.int32),
          "dihedral_params": jnp.array(atomic_sys.dihedral_params) if atomic_sys.dihedral_params is not None else jnp.zeros((0, 3)),
          "impropers": jnp.array(atomic_sys.impropers) if atomic_sys.impropers is not None else jnp.zeros((0, 4), dtype=jnp.int32),
          "improper_params": jnp.array(atomic_sys.improper_params) if atomic_sys.improper_params is not None else jnp.zeros((0, 3)),
  ...
  if exclusion_spec is None:
      ...
      logger.info("Built ExclusionSpec: %d 1-2/1-3 pairs, %d 1-4 pairs",
                  len(exclusion_spec.idx_12_13), len(exclusion_spec.idx_14))
  ...
  if spec.use_neighbor_list and spec.box is not None:
      neighbor_fn = nl.make_neighbor_list_fn(displacement_fn, np.array(spec.box), spec.neighbor_cutoff)
  ...
  logger.info("Neighbor list update: every %d steps (%.1f fs), %d updates per save",
              steps_per_update_actual, steps_per_update_actual * spec.step_size_fs,
  ...
  def scan_fn_with_neighbor(carrier, _):
      ...
      curr_state, nbrs = jax.lax.fori_loop(0, neighbor_updates_per_save, outer_step, (curr_state, nbrs))
