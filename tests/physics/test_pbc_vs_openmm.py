...
def test_pme_energy_matches_openmm_two_particles():
  ...
  omm_system.setDefaultPeriodicBoxVectors(
    openmm.Vec3(box_size, 0, 0), openmm.Vec3(0, box_size, 0), openmm.Vec3(0, 0, box_size)
  )
  ...
  omm_sys_ref.setDefaultPeriodicBoxVectors(
    openmm.Vec3(box_size, 0, 0), openmm.Vec3(0, box_size, 0), openmm.Vec3(0, 0, box_size)
  )
  omm_sys_ref.addParticle(1.0)
  omm_sys_ref.addParticle(1.0)
  omm_sys_ref.addForce(nonbonded_ref)

  ctx_ref = openmm.Context(
    omm_sys_ref, openmm.VerletIntegrator(0.001), openmm.Platform.getPlatformByName("Reference")
  )
  ctx_ref.setPositions(pos_array)
  e_ref = (
    ctx_ref.getState(getEnergy=True).getPotentialEnergy().value_in_unit(unit.kilocalories_per_mole)
  )
