...
def test_nve_energy_conservation():
  ...
  final_R = simulate.run_nve(energy_fn, R_init, steps=100, dt=1e-3, mass=1.0)
  ...
  init_fn, apply_fn = simulate.simulate.nve(
    energy_fn, shift_fn, ts
...
def test_brownian_execution():
  ...
  final_R = simulate.run_brownian(
    energy_fn, R, steps=100, dt=1e-3, temperature=300.0, gamma=0.1, mass=1.0
  )
  assert final_R.shape == R.shape
