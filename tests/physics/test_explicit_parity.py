...
class TestNonbondedEnergies:
  ...
  def test_pme_energy_finite(self, parameterized_protein):
    ...
    pme_fn = pme.make_pme_energy_fn(protein.charges, box, grid_points=PME_GRID, alpha=PME_ALPHA)
...
class TestOpenMMParity:
  ...
  def test_bond_energy_parity(self, shared_prepared_system):
    ...
    context = openmm.Context(
      bond_system, integrator, openmm.Platform.getPlatformByName("Reference")
    )
  ...
  def test_angle_energy_parity(self, shared_prepared_system):
    ...
    integrator = openmm.VerletIntegrator(0.001 * unit.picoseconds)
    context = openmm.Context(
      angle_system, integrator, openmm.Platform.getPlatformByName("Reference")
    )
    context.setPositions((positions_A * 0.1) * unit.nanometer)
  ...
  def test_dihedral_energy_parity(self, shared_prepared_system):
    ...
    integrator = openmm.VerletIntegrator(0.001 * unit.picoseconds)
    context = openmm.Context(
      torsion_system, integrator, openmm.Platform.getPlatformByName("Reference")
    )
    context.setPositions((positions_A * 0.1) * unit.nanometer)
    ...
    print(
      f"Dihedral Energy - NumPy: {e_dihedrals_np:.2f}, OpenMM: {e_dihedrals_omm:.2f}, Diff: {diff:.4f}"
    )
