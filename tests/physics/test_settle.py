...
class TestSETTLEPositions:
  ...
  def test_single_water_constraints(self):
    ...
    R_constrained = settle.settle_positions(
      R_new, R_old, water_indices, r_OH=settle.TIP3P_ROH, r_HH=settle.TIP3P_RHH
    )
    ...
    assert jnp.abs(r_OH1 - settle.TIP3P_ROH) < 0.01, (
      f"O-H1 = {r_OH1:.4f}, expected {settle.TIP3P_ROH}"
    )
    assert jnp.abs(r_OH2 - settle.TIP3P_ROH) < 0.01, (
      f"O-H2 = {r_OH2:.4f}, expected {settle.TIP3P_ROH}"
    )
  ...
  def test_settle_with_pbc_crossing(self):
    ...
    R_constrained = settle.settle_positions(R_new, R_old, water_indices, box=box)
    ...
    assert jnp.abs(r_OH1 - settle.TIP3P_ROH) < 0.05, (
      f"O-H1 = {r_OH1:.4f}, expected {settle.TIP3P_ROH}"
    )
    assert jnp.abs(r_OH2 - settle.TIP3P_ROH) < 0.05, (
      f"O-H2 = {r_OH2:.4f}, expected {settle.TIP3P_ROH}"
    )
