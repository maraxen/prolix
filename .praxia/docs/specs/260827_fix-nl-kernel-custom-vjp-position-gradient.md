...
and calls `jax.grad`/`jax.value_and_grad` through it twice when `spec.use_neighbor_list=True`:
once to size FIRE's `dt_start` (`simulate.py:645-653`) and once in the steepest-descent
preconditioner (`simulate.py:177`). This is a documented, **recommended** path — the
...
silently degrading real minimization runs on that path today (near-zero `dt_start` sizing,
near-zero steepest-descent forces — a near no-op) rather than having "no other surface." See
...
| Validation gate 4 — `run_simulation`/`use_neighbor_list=True` (**new, round-1 finding 6, required**) | `src/prolix/simulate.py::run_simulation` calls `jax.grad`/`jax.value_and_grad` through these exact kernels today when `spec.use_neighbor_list=True` (FIRE `dt_start` sizing, `simulate.py:645-653`; steepest-descent preconditioner, `simulate.py:177`) — a real, documented, **recommended** path (explicit-solvent runbook, a notebook, and a script all use it), not dead code. Run `tests/physics/test_robust_minimization.py` (which already contrasts `use_neighbor_list=True` vs `False`, line 133) before and after the fix. The fix is expected to **change** this path's behavior (today's near-zero `dt_start`/near-zero steepest-descent forces are themselves a symptom of this bug, not a feature) — confirm the change is a genuine improvement (energy actually decreases during minimization, `dt_start` is no longer pinned to its ceiling) rather than a new failure mode, and update that test's expectations if it encodes the old (buggy) near-zero-force behavior as correct. |
...
- Re-run `tests/physics/test_1vii_nl_nonbonded_parity.py` (from #4383), the existing water/flash probes, and `tests/physics/test_robust_minimization.py` (`use_neighbor_list=True` case) as validation gates 2, 3, and 4.
...
6. Given the fix, when `tests/physics/test_robust_minimization.py` is re-run with `use_neighbor_list=True`, then minimization genuinely progresses (energy decreases, `dt_start` is not pinned to its ceiling) rather than reproducing the pre-fix near-zero-force near-no-op — and if that test currently asserts the old (buggy) behavior as correct, it is updated accordingly, not left silently contradicting the fix.
...
path's near-zero-`dt_start`/near-zero-steepest-descent-force behavior, not just re-blocking
...
- Production consumer requiring Validation gate 4 (round-1 finding 6): `src/prolix/simulate.py::run_simulation` (~177, ~645-653), `src/prolix/physics/system.py::make_energy_fn` (~191-236), `tests/physics/test_robust_minimization.py`
