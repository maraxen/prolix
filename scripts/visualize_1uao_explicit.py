...
def main():
    ...
    # Bounds - use box size from PDB CRYST1 if available for proper visualization
    if atom_array.box is not None:
        box = atom_array.box
        if box.ndim == 3:
            box = box[0]
        box_size = np.array([box[0,0], box[1,1], box[2,2]])
        ...
        max_range = np.max(max_bound - min_bound) / 2 * 1.1
